#!/usr/bin/env python3
"""Tkinter GUI for the Dattorro Plate Reverb processor."""
...
from dattorro import (
    DEFAULT_BANDWIDTH,
    DEFAULT_DAMPING,
    DEFAULT_DECAY,
    DEFAULT_DRY,
    DEFAULT_INPUT_DIFFUSION,
    DEFAULT_PRE_DELAY,
...
class DattorroGUI(tk.Tk):
    """Main application window."""

    def __init__(self) -> None:
        super().__init__()
        self.title("Dattorro Plate Reverb")
    ...
    def _build_ui(self) -> None:
        ...
        params = [
            ("pre_delay", "Pre-Delay", DEFAULT_PRE_DELAY),
            ("bandwidth", "Bandwidth", DEFAULT_BANDWIDTH),
            ("damping", "Damping", DEFAULT_DAMPING),
            ("decay", "Decay", DEFAULT_DECAY),
            ("input_diffusion", "Input Diffusion", DEFAULT_INPUT_DIFFUSION),
            ("wet", "Wet", DEFAULT_WET),
            ("dry", "Dry", DEFAULT_DRY),
            ("stereo_width", "Stereo Width", DEFAULT_STEREO_WIDTH),
        ]
    ...
    def _apply_worker(self, input_path: str, output_path: str, params: dict) -> None:
        try:
            apply_reverb(
                ...
                decay=params["decay"],
                input_diffusion=params["input_diffusion"],
                wet=params["wet"],
                dry=params["dry"],
