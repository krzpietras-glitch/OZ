#!/usr/bin/env python3
"""Tkinter GUI for the Convolution Reverb processor."""
...
class ConvolutionReverbGUI(tk.Tk):
    """Main application window."""

    def __init__(self) -> None:
        super().__init__()
        self.title("Convolution Reverb")
    ...
    def _build_ui(self) -> None:
        ...
        # --- IR file selection -----------------------------------------------
        ir_frame = tk.LabelFrame(self, text="Impulse Response", padx=8, pady=4)
        ir_frame.pack(fill="x", **pad)

        self._ir_var = tk.StringVar(value="Using synthetic IR")
        tk.Label(ir_frame, textvariable=self._ir_var, anchor="w", width=50).pack(
            side="left", fill="x", expand=True
        )
        tk.Button(ir_frame, text="Browse…", command=self._browse_ir).pack(side="right")
        ...
        params = [
            ("ir_length", "IR Length", DEFAULT_IR_LENGTH),
    ...
    def _browse_ir(self) -> None:
        path = filedialog.askopenfilename(
            title="Select IR WAV file",
            filetypes=[("WAV files", "*.wav"), ("All files", "*.*")],
        )
        if path:
            self._ir_path = path
            self._ir_var.set(os.path.basename(path))
            self._set_status(f"IR loaded: {os.path.basename(path)}")
