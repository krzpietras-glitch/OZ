"""Tests for the Moorer reverb implementation."""
...
# ---------------------------------------------------------------------------
# TappedDelayLine tests
# ---------------------------------------------------------------------------
class TestTappedDelayLine:
    def test_delays_signal(self):
        """Each tap should produce output after its delay length."""
        tap_delays = [5, 10]
        tap_gains = [1.0, 1.0]
        tdl = TappedDelayLine(tap_delays, tap_gains, gain=1.0)
        impulse = np.zeros(20)
        impulse[0] = 1.0
        out = np.array([tdl.process(s) for s in impulse])

        # Before the shortest tap, output is zero
        assert np.all(out[:5] == 0.0)
        # At sample 5, the first tap fires
        assert out[5] == 1.0
        # At sample 10, the second tap fires
        assert out[10] == 1.0

    def test_gain_scaling(self):
        """The gain parameter should scale all tap outputs."""
        tap_delays = [5]
        tap_gains = [0.8]
        tdl = TappedDelayLine(tap_delays, tap_gains, gain=0.5)
        impulse = np.zeros(10)
        impulse[0] = 1.0
        out = np.array([tdl.process(s) for s in impulse])

        assert abs(out[5] - 0.4) < 1e-12  # 0.8 * 0.5

    def test_zero_gain_produces_silence(self):
        """With gain=0, the tapped delay line should output silence."""
        tap_delays = [5, 10]
        tap_gains = [0.8, 0.6]
        tdl = TappedDelayLine(tap_delays, tap_gains, gain=0.0)
        impulse = np.zeros(20)
        impulse[0] = 1.0
        out = np.array([tdl.process(s) for s in impulse])
...
# ---------------------------------------------------------------------------
# LowpassCombFilter tests
# ---------------------------------------------------------------------------
class TestLowpassCombFilter:
    def test_delays_signal(self):
        """With zero feedback the comb filter acts as a pure delay."""
        delay = 10
        comb = LowpassCombFilter(delay, feedback=0.0, damping=0.0)
        impulse = np.zeros(25)
        impulse[0] = 1.0
    ...
    def test_feedback_produces_decaying_echoes(self):
        ...
        delay = 10
        comb = LowpassCombFilter(delay, feedback=0.5, damping=0.0)
        impulse = np.zeros(50)
        impulse[0] = 1.0
        out = np.array([comb.process(s) for s in impulse])
    ...
    def test_damping_attenuates_highs(self):
        ...
        delay = 10
        comb_no_damp = LowpassCombFilter(delay, feedback=0.7, damping=0.0)
        comb_hi_damp = LowpassCombFilter(delay, feedback=0.7, damping=0.9)
...
class TestMoorerReverb:
    ...
    def test_dry_only_preserves_input(self):
        ...
        result = MoorerReverb(wet=0.0, dry=1.0).process(mono)
    ...
    def test_wet_adds_reverb_tail(self):
        ...
        impulse[0] = 0.8
        result = MoorerReverb(room_size=0.8, wet=0.5, dry=0.5).process(impulse)
        tail_energy = np.mean(result[2000:] ** 2)
        assert tail_energy > 1e-6

    def test_silence_in_silence_out(self):
        silence = np.zeros(500)
        result = MoorerReverb().process(silence)
        assert np.allclose(result, 0.0)

    def test_early_reflections_add_energy_before_late_reverb(self):
        """With early_reflections > 0 there should be energy in the output
        before the comb filter delays kick in (< 2205 samples at 44100 Hz).
        With early_reflections = 0 that region should be near-silent (only dry).
        """
        impulse = np.zeros(3000)
        impulse[0] = 0.8

        # early_reflections = 0 → only comb/allpass (late) and dry
        result_no_er = MoorerReverb(
            wet=1.0, dry=0.0, early_reflections=0.0
        ).process(impulse)
        # early_reflections = 1.0 → tapped delay taps fire in 190..2021 range
        result_er = MoorerReverb(
            wet=1.0, dry=0.0, early_reflections=1.0
        ).process(impulse)

        # Energy in the early region (samples 200-2000) before combs respond
        early_region = slice(200, 2000)
        energy_no_er = np.sum(result_no_er[early_region] ** 2)
        energy_er = np.sum(result_er[early_region] ** 2)
        assert energy_er > energy_no_er
