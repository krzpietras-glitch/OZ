#!/usr/bin/env python3
"""Freeverb reverb implementation using only numpy and scipy.

Freeverb uses 8 parallel feedback comb filters (with lowpass damping)
fed into 4 series allpass filters, with stereo spread achieved by
offsetting the right-channel delay lines.
...
# ---------------------------------------------------------------------------
# Freeverb constants (Jezar's original values at 44100 Hz)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
class Freeverb:
    """Stereo Freeverb reverb processor.
...
def apply_reverb(
    ...
    """Read a WAV file, apply Freeverb, and write the result.
    ...
    reverb = Freeverb(sample_rate, room_size, damping, wet, dry, width)
...
def main():
    parser = argparse.ArgumentParser(
        description="Apply Freeverb reverb to a WAV file."
    )
